...
import           Foreign.Lua hiding (error)
...
factorioDataSubmodulePath moduleName =
    pack $ unpack factorioDataPathRoot </> unpack moduleName

allFactorioDataModulePaths :: [ByteString]
allFactorioDataModulePaths = map
    factorioDataModulePattern
    (factorioDataPathRoot : map factorioDataSubmodulePath pathModules)
    where pathModules = ["base", "core", pack $ "core" </> "lualib"]
...
addFactorioDataPath = do
    ...
    push $ intercalate
        ";"
        (currentPath : "./lua/?.lua" : allFactorioDataModulePaths)
    setfield (-2) "path"
    pop 1
...
polyfill = do
    ...
    pop 1
    where mathPowPolyfill x y = return (x ** y) :: Lua Float
...
instance ToJSON LuaValue where
    ...
    toJSON (LVTable kv) | hasOnlyIntKeys = toJSON $ snd <$> kv
